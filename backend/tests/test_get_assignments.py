...
def test_get_asssignments_pass():
    ...
    x = requests.get(f"https://dev.dakshsrivastava.com/assignments/class/{class_id}", verify=False)
    ...
    conn.commit()
    conn.close()
    assert (res[0][0] == assignment_id), f"Res is {res}"
