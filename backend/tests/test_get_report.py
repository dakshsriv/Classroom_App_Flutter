...
def test_get_report():
    ...
    conn.commit()
    x = requests.post(f"https://dev.dakshsrivastava.com/submissions/{class_id}", verify=False)
    report = json.loads(x.text)
    ...
    conn.commit()
    conn.close()
    assert (report[0]['username']['Test Assignment'][0][0] == 0), report