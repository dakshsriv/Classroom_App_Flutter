...
def test_get_status():
    ...
    conn.commit()
    x = requests.post(f"https://dev.dakshsrivastava.com/status/", json={"student_id":student_id, "assignment_id":assignment_id}, verify=False)
    status = json.loads(x.text)

    cursor.execute("SELECT * FROM SubmitAssignments")
    z = cursor.fetchall()
